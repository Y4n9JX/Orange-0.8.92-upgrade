...
class HttpService {
  ...
  HttpService._internal(this.baseUrl, this.httpConfig, this._tokenManager);
  ...
  void _initializeDio() {
    ...
    (_dio.httpClientAdapter as IOHttpClientAdapter).createHttpClient = () {
      ...
      // XBoard SDK owns its network route. Do not let FlClash's global
      // HttpOverrides/user proxy switch implicitly hijack these requests.
  ...
  dynamic _deobfuscateResponse(Response response) {
    try {
      ...
      if (httpConfig.enableAutoDeobfuscation &&
        ...
        // 检测是否包含混淆前缀
        final containsObfuscationPrefix = responseText.contains(
          httpConfig.obfuscationPrefix!,
        );

        if (containsObfuscationPrefix) {
          // 反混淆：移除混淆前缀
          final deobfuscated = responseText.replaceAll(
            httpConfig.obfuscationPrefix!,
            '',
          );
  ...
  bool _verifyCertificate(X509Certificate cert, String host, int port) {
    try {
      ...
      // 比较证书内容（忽略空白字符差异）
      final expectedNormalized = _expectedCertificatePem!.replaceAll(
        RegExp(r'\s+'),
        '',
      );
