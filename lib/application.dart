...
class ApplicationState extends ConsumerState<Application> {
  ...
  Future<void> _initializeXBoard() async {
    try {
      ...
      if (!mounted || !ref.read(initializationProvider).isReady) {
        return;
      }
      await ref.read(xboardUserProvider.notifier).quickAuth();
    } catch (e) {
      commonPrint.log('[Application] XBoard initialization skipped: $e');
    }
  }
  ...
  GoRouter _buildRouter(UserAuthState userState) {
    return GoRouter(
      ...
      redirect: (context, state) {
        ...
        if (isAuthenticated && (isLoginPage || isLoadingPage)) {
          return '/';
        }
