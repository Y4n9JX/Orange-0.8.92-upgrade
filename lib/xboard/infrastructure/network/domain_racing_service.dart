...
class DomainRacingService {
  ...
  static Future<DomainRacingResult?> raceSelectFastestDomain(
    ...
    _logger.info(
      '[域名竞速] 开始竞速测试 ${domains.length} 个域名${proxies.isNotEmpty ? '（每个测试直连+${proxies.length}个代理）' : ''}，共 $testCount 个测试',
    );
        ...
        futures[i]
            .then((result) {
              if (!completer.isCompleted && result.success) {
                // 第一个成功的获胜
                _logger.info(
                  '[域名竞速] 🏆 域名 #$i (${result.domain}) [${_maskConnectionType(result)}] 获胜！响应时间: ${result.responseTime}ms',
                );
                ...
                if (result.error != null) {
                  final connectionType = _maskConnectionType(result);
                  _logger.info(
  ...
  static Future<DomainTestResult> _testSingleDomain(
    ...
    try {
      ...
      // XBoard 竞速必须只比较“真直连”和“预置 SOCKS5”，不能被
      // FlClash 全局 HttpOverrides / 用户当前代理节点污染。
      XBoardNetworkUtils.bypassGlobalProxy(client);

      // 如果使用代理，配置 SOCKS5 代理
      if (useProxy && proxyUrl != null) {
        final proxyConfig = XBoardNetworkUtils.parseProxyConfig(proxyUrl);
        XBoardNetworkUtils.applySocks5Proxy(client, proxyUrl);
        _logger.info(
      ...
      if (_isIpWithPort(withoutProtocol)) {
        ...
        final apiUserAgent = await UserAgentConfig.get(
        ...
        final domainUserAgent = await UserAgentConfig.get(
          UserAgentScenario.domainRacingTest,
        );
      ...
      if (response.statusCode >= 200 && response.statusCode < 400) {
        ...
        _logger.info(
          '[域名竞速] 域名 #$index ($domain) 返回状态码: ${response.statusCode}',
        );
        return DomainTestResult.failure(
          domain,
  ...
  static Future<List<DomainTestResult>> testAllDomains(
    ...
    final List<Future<DomainTestResult>> futures = domains.asMap().entries.map((
      entry,
    ) {
