...
class XBoardProfileImportService {
  ...
  Future<void> _cleanOldUrlProfiles() async {
    try {
      ...
      for (final profile in urlProfiles) {
        _logger.debug(
          '删除旧的URL配置: ${profile.label.isNotEmpty ? profile.label : profile.id}',
        );
  ...
  Future<Profile> _downloadAndValidateProfile(String url) async {
    try {
      ...
      _logger.info(
        '配置下载和验证成功: ${profile.label.isNotEmpty ? profile.label : profile.id}',
      );
      return profile;
  ...
  Future<Profile> _downloadEncryptedProfile(String url) async {
    try {
      ...
      try {
        ...
        _logger.debug(
          '🔄 Fallback - 从URL提取到token: ${token.substring(0, 8)}...',
        );
        result = await EncryptedSubscriptionService.getSubscriptionSmart(
          token,
          preferEncrypt: preferEncrypt,
  ...
  Future<void> _addProfile(Profile profile) async {
    try {
      ...
      _logger.info(
        '✅ 已设置为当前配置: ${profile.label.isNotEmpty ? profile.label : profile.id}',
      );
      ...
      _logger.info(
        '配置添加成功: ${profile.label.isNotEmpty ? profile.label : profile.id}',
      );
    } catch (e) {
