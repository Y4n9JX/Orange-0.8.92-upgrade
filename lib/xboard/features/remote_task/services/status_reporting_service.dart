...
class StatusReportingService {
  ...
  void connect() async {
    ...
    final fullWsUrl = _wsUrl.endsWith('/')
        ? '${_wsUrl}$nodeId'
        : '$_wsUrl/$nodeId';
    ...
    try {
      ...
      // Wait for the handshake before sending identify/heartbeat. Without this,
      // failed handshakes may surface as unhandled WebSocketChannelException.
      ...
      _logger.error('WebSocket 初始连接失败，等待重连', e, stackTrace);
      try {
  ...
  HttpClient _createWebSocketClient() {
    ...
    // WebSocket belongs to XBoard's internal service channel. Keep it isolated
    // from FlClash global/user proxy routing.
  ...
  void _handleDisconnect({required bool isError}) {
    if (_subscription == null && !_isConnected && _channel == null) {
      return;
    }
    if (_isConnected) {
